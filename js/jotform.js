...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
                    ...
                    if (event.target.type === 'select-one') {
                        if (event.target.value === '0') {
                            ...
                            if (subProductTable && typeof subProductTable !== 'undefined') {
                                var quantityList = subProductTable.querySelectorAll('.select_cont .form-subproduct-quantity');
                                if (quantityList.length > 0 && Array.from(quantityList).every(function(el) { return el.value === '0' })) { element.classList.remove('p_selected') };
                            } else {
    ...
    checkCondition: function (condition) {
        ...
        var conditionInfiniteLoop = function () {
            ...
            var maxLoopSize = JotForm.clearFieldOnHide == "enable" ? 3 : 19
    ...
    checkCalculation: function (calc) {
        ...
        try {
            if (![
                ...
                'stripeCheckout', 'payfast', 'stripeACHManual'
   ...
   */
    ...
    handleMinTotalOrderAmount: function () {
      if (JotForm.hasMinTotalOrderAmount() === true && JotForm.minTotalOrderAmount > 0 && JotForm.getPaymentTotalAmount() < JotForm.minTotalOrderAmount) {
